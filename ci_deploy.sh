...
REPO_PATH={$REPO_PATH:-"https://github.com/applariat/acme-air/archive"}
...
#APL STACK variables
#Required as env variable inputs from CI
APL_STACK_NAME=${APL_STACK_NAME:?Missing required env var}
APL_RELEASE_VERSION=${APL_RELEASE_VERSION:?Missing required env var}
APL_COMPONENT_NAME=${APL_COMPONENT_NAME:?Missing required env var}
...
if ! [ `command -v jq` ]; then
    #Let's install jq
    echo "This script requires jq tool, installing"
    JQ_TOOL=jq-linux64
    if [[ "$OSTYPE" == "darwin"* ]]; then
